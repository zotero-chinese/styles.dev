...
function get_files(base, defaule_date_file, override_data_file, defaule_cite_file, override_cite_file) {
    ...
    files.forEach(function (file_name) {
        ...
        if (stats.isFile() && path.extname(file_full_path) == ".csl") {
            ...
            list.push([file_full_path, base, data_file_path, file_name, cite_file_path])
        }
...
csls = get_files(csl_base, default_data_file, override_data_file, defaule_cite_file, override_cite_file)
console.log(csls);
for (var csl of csls) {
    // console.log(csl[0], csl[2])
    itemdata = get_info(csl[0], csl[2], csl[3]);
