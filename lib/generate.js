    ...
    files.forEach(function (file_name) {
        ...
        if (stats.isFile() && path.extname(file_full_path) == ".csl") {
            ...
            let pathResult = {
                ...
                'github_link': data_file['github_repo'] + './tree/main/' + path.relative('../', file_full_path).replace(/\\/ig, "/"),
                'github_raw': data_file['github_repo'] + './raw/main/' + path.relative('../', file_full_path).replace(/\\/ig, "/"),
...
if (args == 'watch'){
    ...
    chokidar.watch('../src/', {
        ...
        get_paths(path.dirname(filePath), config).forEach((self) => {
            console.log('Processing ' + self['file_full_path'])
            itemdata = get_info(self['file_full_path'],
                self['sample_data_path'],
                self['sample_cite_path'],
